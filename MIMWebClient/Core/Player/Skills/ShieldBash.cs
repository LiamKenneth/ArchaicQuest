...
namespace MIMWebClient.Core.Player.Skills
{
    ...
    public class ShieldBash: Skill
    {
        ...
        public void StartBash(IHubContext context, PlayerSetup.Player player, Room room, string target = "")
        {
            ...
            var hasSkill = Skill.CheckPlayerHasSkill(player, ShieldBashAb().Name);
            ...
            if (player.Equipment.Shield.Equals("Nothing"))
            {
                context.SendToClient("You need a shield to shield bash.", player.HubGuid);
                return;
            }
            ...
            if (_target != null)
            {
                ...
                if (player.MovePoints < ShieldBashAb().MovesCost)
                {


                    context.SendToClient("You are too tired to use Shield Bash.", player.HubGuid);
                    player.ActiveSkill = null;

                    return;
                }

                player.MovePoints -= ShieldBashAb().MovesCost;
                ...
                var skill = player.Skills.FirstOrDefault(x => x.Name.Equals("Shield Bash"));
                if (skill == null)
                {
                    player.ActiveSkill = null;
                    return;
                ...
                {
                    player.ActiveSkill = null;
                    PlayerSetup.Player.LearnFromMistake(player, ShieldBashAb(), 250);
