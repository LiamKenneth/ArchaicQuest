...
namespace MIMWebClient.Core.Player.Skills
{
    ...
    public class SpinKick : Skill
    {
        private static bool _taskRunnning = false;
        public static SpinKick KickSkill { get; set; }

        public static SpinKick KickAb()
        {


            var skill = new SpinKick
            {
                Name = "Spin Kick",
                CoolDown = 0,
                Delay = 0,
                ...
                Syntax = "Spin kick <Target>"
            };


            var help = new Help
            {
                Syntax = "spin Kick <Victim>",
        ...
        public void StartKick(IHubContext context, PlayerSetup.Player player, Room room, string target = "")
        {
            ...
            if (_target != null)
            {
                ...
                {
                    context.SendToClient("You can't spinkick them as they are dead.", player.HubGuid);
                ...
                if (player.MovePoints < KickAb().MovesCost)
                {
                    ...
                    context.SendToClient("You are too tired to use spin kick.", player.HubGuid);
                    player.ActiveSkill = null;
                ...
                var skill = player.Skills.FirstOrDefault(x => x.Name.Equals("Spin Kick"));
                if (skill == null)
                {
                    player.ActiveSkill = null;
                    return;
                ...
                {
                    player.ActiveSkill = null;
                    HubContext.Instance.SendToClient("You don't see an opportunity to Spin kick.",
                        player.HubGuid);
